...
export async function POST(request: NextRequest) {
  try {
    const { user_id, appointment_type, start_time }: ForceInsertRequest = await request.json()
    ...
    if (
      ...
      return NextResponse.json({
        success: false,
        error: 'appointment_type must be onboarding_screening or onboarding_consultation'
      }, { status: 400 })
    }

    const { data, error } = await supabaseAdmin.rpc('test_insert_appointment', {
      p_user_id: user_id,
    ...
    console.error('Error force-inserting appointment:', error)
    return NextResponse.json({
      success: false,
